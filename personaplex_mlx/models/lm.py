...
class LmConfig:
    ...
    def from_config_dict(cls, data: dict) -> "LmConfig":
        ...
        return LmConfig(
            ...
            audio_tokens_per_stream=data.get("audio_tokens_per_stream", 8),
        )
...
class DepFormer(nn.Module):
    ...
    def sample(
        ...
        cfg_coef: float = 1.0,
        forced_audio_tokens: mx.array | None = None,
        forced_audio_mask: mx.array | None = None,
    ) -> mx.array:
        tokens = []
        ...
        for slice in self.slices:
            ...
            sampled_token, _ = sampler(logits)
            if forced_audio_tokens is not None:
                forced_token = forced_audio_tokens[:, len(tokens) : len(tokens) + 1]
                forced_mask = forced_audio_mask[:, len(tokens) : len(tokens) + 1]
                last_token = mx.where(forced_mask, forced_token, sampled_token)
            else:
...
class Lm(nn.Module):
    ...
    @property
    def audio_delays(self) -> list[int]:
        return self.cfg.audio_delays

    @property
    def all_delays(self) -> list[int]:
        return [self.cfg.text_delay] + self.cfg.audio_delays
    ...
    def embed_codes(
        ...
        xs = self.text_emb(token_ids[:, 0, :])
        for cb_idx, emb in enumerate(self.audio_embs):
            cb_tokens = token_ids[:, cb_idx + 1, :].transpose(1, 0)
            cb_emb = emb(cb_tokens).transpose(1, 0, 2)
            xs = xs + cb_emb
        if ct is not None:
    ...
    def forward_codes(
        ...
        xs = self.embed_codes(token_ids, ct=ct)
        return self.forward_embeddings(xs, cross_attention_src=cross_attention_src)
    ...
    def _sample(
        ...
        for token_ids, emb in zip(audio_token_ids, self.audio_embs):
            _emb = emb(token_ids).transpose(1, 0, 2)
            xs = xs + _emb
        if ct is not None:
            xs = xs + mx.expand_dims(ct.tensor, axis=1)
        if cfg_coef != 1:
        ...
        if len(self.depformer.slices) > 0:
            audio_tokens = self.depformer.sample(
                ...
                self.depformer_cache,
                cfg_coef=cfg_coef,
                forced_audio_tokens=forced_audio_tokens,
                forced_audio_mask=forced_audio_mask,
            )
    ...
    def warmup(self, ct: ConditionTensor | None = None):
        text, audio = self.sample(
            ...
            [mx.array([[0]])] * self.cfg.audio_codebooks,
            text_sampler=sampling.Sampler(),
            audio_sampler=sampling.Sampler(),
            ct=ct,
        )
